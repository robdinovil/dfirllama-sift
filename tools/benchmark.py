"""
DFIRLlama Benchmark — mide calidad del motor NL→SQL y la tasa de alucinaciones.

Métricas implementadas:

  Score                               PASS/FAIL ratio (binario)
  Hallucination Rate                  alucinaciones no resueltas / total
  SCR — Self-Correction Rate          % de alucinaciones auto-corregidas por el validador
  Latencia                            avg y p95 por pregunta
...
_QUESTION_TIMEOUT_S = 600  # 10 min hard limit por pregunta (safe en CPU-only)


# ── Ground truth — 25 preguntas B01-B25 ──────────────────────────────────────
# Cada entrada:
#   question          — pregunta en lenguaje natural
#   expected_cols     — columnas que DEBEN aparecer en el resultado
#   must_contain      — keywords que deben estar en el SQL generado
...
#   applies_to        — tablas requeridas para que aplique esta pregunta
...
BENCHMARK_QUESTIONS = [
    ...
    # ── SANS FIND EVIL — investigación de incidente ───────────────────────────
    {
        "id": "B21",
...
class QuestionResult:
    ...
    passed: bool
    hallucination_type: str | None        # tipo final (después de retry)
    ...
    issues: list[str]
    elapsed_s: float
    tus_score: float = 0.0    # TUS — Task-level Understanding Score (DFIR-Metric)
...
class BenchmarkReport:
    ...
    passed: int
    failed: int
    hallucinations: dict       # {structural, referential, syntax} — no resueltos
    ...
    def self_correction_rate(self) -> float:
        """% de alucinaciones detectadas que fueron auto-corregidas."""
...
def _compute_tus(q: dict, sql_lower: str, rows: int | None) -> float:
    ...
    Fuente: DFIR-Metric (arxiv 2505.19973), adaptado para NL→SQL forense.

    C1: tablas applies_to presentes en el SQL
    C2: expected_cols presentes en SELECT/WHERE
    C3: fracción de must_contain keywords en el SQL
    C4: resultado en rango [min_rows, max_rows]
    """
    scores: list[float] = []
...
def _compute_context_recall(question: str, q: dict, analyst) -> float:
    """
    NonLLM Context Recall — % de términos clave cubiertos por los top-5 QA pairs
    recuperados por BM25. Mide la calidad del retriever independientemente del LLM.
...
def _ask_with_timeout(analyst, question: str, timeout_s: int = _QUESTION_TIMEOUT_S):
    """Run analyst.ask() in a daemon thread with a hard wall-clock timeout."""
    bucket: dict = {}

    def _target():
        try:
            bucket["result"] = analyst.ask(question)
...
def run(db_path: str, model: str = "",
        save_json: bool = True, out_path: str | None = None) -> BenchmarkReport:
    """
    Run the DFIRLlama benchmark against a normalized evidence SQLite.

    Args:
        db_path:   Path to normalized SQLite (from ingest_evidence_dir_tool).
        model:     Model name for the report header (auto-detected if empty).
        save_json: Write JSON report to disk.
        out_path:  JSON output path (default: <db_stem>_benchmark.json).

    Returns:
        BenchmarkReport with all 6 metrics.
    """
    ...
    analyst.train()
    analyst._active_tables = analyst._detect_active_tables(
    ...
    print(f"\n{CYAN}{BOLD}{'─'*68}{RESET}")
    print(f"{CYAN}{BOLD}  DFIRLlama Benchmark — {Path(db_path).stem}{RESET}")
    print(f"{CYAN}{BOLD}{'─'*68}{RESET}")
    print(f"  {'ID':<5} {'Categoría':<15} {'Estado':<12} {'Alucinación':<18} {'s':>5}  Pregunta")
    print(f"  {'─'*4} {'─'*13} {'─'*10} {'─'*16} {'─'*5}  {'─'*30}")
    ...
    for q in BENCHMARK_QUESTIONS:
        ...
        if exc is not None:
            ...
            print(f"  {q['id']:<5} {q['category']:<15} {RED}{label:<10}{RESET}  ─"
                  f"                    {elapsed:>5}s  {q['question'][:35]}")
            results.append(QuestionResult(
                ...
                self_corrected=False, issues=[str(exc)], elapsed_s=elapsed,
            ))
            continue
        ...
        print(f"  {q['id']:<5} {q['category']:<15} {status_str:<20} {h_str:<28} "
              f"{elapsed:>5}s  {q['question'][:35]}")
    ...
    h_types: dict[str, int] = {"structural": 0, "referential": 0, "syntax": 0}
    for r in results:
...
def _print_summary(r: BenchmarkReport) -> None:
    ...
    print(f"\n{CYAN}{BOLD}{'─'*68}{RESET}")
    print(f"{CYAN}{BOLD}  RESUMEN  —  modelo: {r.model}{RESET}")
    print(f"  Score  (PASS/FAIL)  : {score_color}{BOLD}{r.passed}/{r.total} ({r.score:.0%}){RESET}")
    print(f"  TUS    (0-1.0)      : {tus_color}{BOLD}{r.tus_avg:.3f}{RESET}"
          f"  ← partial credit [DFIR-Metric]")
    print(f"  RS     (0-1.0)      : {rs_color}{BOLD}{r.reliability_score:.3f}{RESET}"
          f"  ← +1 correct / -2 wrong [DFIR-Metric]")
    print(f"  CCR    (0-1.0)      : {ccr_color}{BOLD}{r.context_recall_avg:.3f}{RESET}"
          f"  ← BM25 retrieval coverage [RAGAS NonLLM]")
    ...
    for cat, stats in sorted(r.by_category.items()):
        s = stats["passed"] / stats["total"] if stats["total"] > 0 else 0.0
